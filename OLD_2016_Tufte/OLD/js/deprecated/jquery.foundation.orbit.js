...
(function ($) {
  ...
  var ORBIT = {
    ...
    init: function (element, options) {
      ...
      $imglides = this.$slides.filter('img');

      if ($imglides.length === 0) {
        this.loaded();
      } else {
        $imglides.bind('imageready', function () {
          imgLoadedCount += 1;
          if (imgLoadedCount === $imglides.length) {
            self.loaded();
          }
        });
      }
...
app.run = function (o) {
	var options = extend(settings, o),
		img = selector(options.img),
		preempted = true;

	for (var l = img.length, i = 0; i < l; i++) {
		var theme = settings.themes.gray;
		var src = img[i].getAttribute("data-src") || img[i].getAttribute("src");
		if (src && !! ~src.indexOf(options.domain)) {
			...
			if (render) {
				...
				img[i].setAttribute("alt", text ? text : theme.text ? theme.text + " [" + dimensions_caption + "]" : dimensions_caption);

				// Fallback
        // img[i].style.width = dimensions.width + "px";
        // img[i].style.height = dimensions.height + "px";
				img[i].style.backgroundColor = theme.background;
				...
				if (!fallback) {
					img[i].setAttribute("src", draw(ctx, dimensions, theme));
				}
			}
		}
	}
